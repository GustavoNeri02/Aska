...
- Em cada interação conversacional, `ConversationService` consulta `TextMemoryRetriever`, que seleciona localmente até cinco memórias por termos normalizados entre a mensagem atual, até dois turnos recentes e o conteúdo persistido. O ranking atribui peso maior a correspondências exatas, depois a plural simples e por fim a variações com radical comum longo e diferença pequena de comprimento. Cada `MemoryMatch` registra score, termos correspondentes e tipos de correspondência; empates preservam a ordem do repositório. `ContextBuilder` inclui somente as memórias selecionadas na mensagem `system`; os motivos permanecem locais até uma pergunta de transparência. Ausência de correspondência não envia memórias. O histórico da sessão segue em mensagens separadas com papéis `user` e `assistant`.
- Das memórias, somente `content` é enviado ao modelo; IDs e metadados permanecem locais por padrão.
- `ConversationService` conserva em memória somente a `MemorySelection` usada na última resposta visível. Perguntas explícitas como “quais memórias você usou?” ou “por que usou essa memória?” reutilizam exatamente essa seleção e apresentam conteúdo, score, termos e tipos de correspondência, sem expor IDs ou executar uma nova busca.
...
- A seleção implementada é lexical e determinística; reconhece plural simples e variações conservadoras por radical longo, mas ainda não reconhece sinônimos, equivalência semântica ou relações implícitas sem semelhança textual.
