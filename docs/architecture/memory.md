...
- A interpretação por modelo está limitada às propostas de alteração do nome, criação explícita e exclusão explícita de uma memória, sem tool calling. JSON inválido, campos ou ações desconhecidos e conteúdo vazio são rejeitados sem produzir proposta. Edição genérica e ações de memória mais amplas permanecem `planned`.
