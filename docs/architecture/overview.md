...
A primeira ação externa controlada abre uma pasta já existente do workspace no Explorador de Arquivos do Windows. `CapabilityProposalRouter` é um port provider-agnostic que apresenta ao modelo um catálogo fechado das proposals implementadas; o catálogo atual contém somente `OpenWorkspaceLocationProposal`. O modelo entende paráfrases e pode propor uma action com argumentos estruturados ou devolver `none`, mas não executa, concede permissões nem informa sucesso. Detectores exatos permanecem apenas como fast paths opcionais, não como vocabulário necessário para acionar a capability.

`NaturalOpenLocationHandler` entrega a proposta à capability, que resolve e confina o caminho localmente, registra um snapshot do diretório e só entrega esse alvo a um `LocationLauncher` injetado depois de confirmação explícita no CLI. Antes da execução, o alvo é validado novamente e uma troca invalida a proposta. O adaptador Windows usa o caminho absoluto de `explorer.exe`, argumentos separados e `shell=False`. A confirmação textual comum foi extraída para o CLI porque memória e desktop agora são consumidores reais; propostas e efeitos continuam em handlers específicos. O roteador não é um registry dinâmico, planner ou executor genérico: novas actions exigem proposal, parsing estrito, handler, política e testes explícitos.
...
A leitura textual confinada ao workspace, a listagem segura de caminhos, sugestões aproximadas locais, a busca textual literal limitada e a abertura confirmada de uma pasta do workspace no Explorador do Windows estão `implemented`. Leitura automática de múltiplos arquivos como contexto integral, busca semântica, indexação persistente e escrita continuam `planned`. Terminal, código, Flutter, browser/web, interação desktop geral, visão, voz, Git/GitHub e organização pessoal também permanecem `planned`. Cada operação adicional deve ter contratos, configuração, permissões e testes próprios apenas quando uma necessidade concreta justificar.
...
Neste primeiro recorte, mensagens que chegam à conversa comum podem consumir uma chamada de classificação no roteador e outra chamada para produzir a resposta. Unificar resposta conversacional e proposal em um único envelope estruturado permanece `planned` e só deve ser feito preservando histórico, contexto, erros e separação entre proposta e execução.
