...
Os pedidos naturais de memória implementados neste estágio são a mudança do nome de Gustavo e a criação explícita de uma memória. `packages/conversation` preserva dois padrões de nome como caminho rápido e usa gates locais para submeter somente paráfrases de nome ou pedidos explícitos de memorização a um intérprete provider-agnostic baseado no modelo. O intérprete produz apenas propostas tipadas (`NameUpdateIntent` ou `AddMemoryIntent`); `PendingMemoryEdit` e `PendingMemoryAdd` mantêm confirmação e cancelamento locais. O CLI apresenta e confirma a proposta sem acessar repository ou datasource, e `MemoryService` permanece responsável pela persistência real. Exclusão natural, edição genérica e captura automática continuam `planned`.
