...
`OpenWorkspaceFileProposal` abre um arquivo regular usando o aplicativo padrão do sistema. Caminhos relativos continuam confinados ao workspace e podem usar `ListFilesCapability` e `suggest_similar_file_paths`; ambiguidades nunca são escolhidas pelo modelo. Um caminho absoluto só é aceito quando aparece explicitamente na proposal e não participa de descoberta. `OpenWorkspaceFileCapability` bloqueia extensões executáveis, scripts, atalhos `.lnk` e assinatura PE, mas permite atalhos de Internet `.url` após confirmação. Ela registra identidade, tamanho e modificação do arquivo e revalida o snapshot antes do efeito. O adapter Windows usa `os.startfile`, sem shell ou comando construído. Pastas, arquivos ausentes, travessia relativa e tipos bloqueados são recusados localmente.
