...
A primeira capability implementada é a leitura de um único arquivo textual. Um gate local limita quando `ModelFileIntentInterpreter` pode classificar o pedido e extrair um caminho por JSON estrito; o modelo não lê o arquivo nem decide permissões. `NaturalFileReadHandler` entrega o caminho a `ReadTextFileCapability`, que retorna um resultado tipado e aceita somente caminhos relativos resolvidos dentro do workspace configurado, rejeitando `file://`, caminhos absolutos POSIX ou Windows, escapes por `..` ou symlink, diretórios, conteúdo não UTF-8, arquivos vazios e arquivos acima de 64 KiB. O conteúdo validado entra em uma mensagem `user` separada apenas na solicitação atual, identificado pela fonte e marcado como dado não confiável; não altera a identidade em `system` nem é copiado para o histórico da conversa.
