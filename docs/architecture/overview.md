...
A edição natural de memória implementada neste estágio permanece restrita à mudança do nome de Gustavo. `packages/conversation` preserva dois padrões textuais determinísticos como caminho rápido e usa um gate local para submeter somente paráfrases relacionadas a nome a um intérprete provider-agnostic baseado no modelo. O intérprete produz apenas uma proposta tipada; a localização de candidatas, a representação como `PendingMemoryEdit`, a confirmação e a mutação por ID e snapshot continuam locais. O CLI apresenta e confirma a proposta sem acessar repository ou datasource, e `MemoryService` permanece responsável pela persistência. Pedidos naturais de memória mais amplos continuam `planned`.
