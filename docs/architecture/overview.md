...
Os pedidos naturais de memória implementados neste estágio são a mudança do nome de Gustavo e a criação ou exclusão explícita de uma memória. `packages/conversation` preserva padrões exatos como caminhos rápidos e usa gates locais separados para submeter somente paráfrases relacionadas a essas ações a um intérprete provider-agnostic baseado no modelo. Detectores determinísticos e intérprete produzem propostas tipadas (`NameUpdateIntent`, `AddMemoryIntent` ou `DeleteMemoryIntent`); propostas imutáveis mantêm confirmação e cancelamento locais. Para exclusão, o CLI seleciona candidatas localmente sem enviar memórias ou IDs ao modelo, e `MemoryService` executa por ID e snapshot. Edição genérica natural, captura automática e ações genéricas continuam `planned`.
