...
Os pedidos naturais de memória implementados neste estágio são a mudança do nome de Gustavo e a criação explícita de uma memória. `packages/conversation` preserva padrões exatos de nome e inclusão como caminhos rápidos e usa gates locais para submeter somente paráfrases relacionadas a essas ações a um intérprete provider-agnostic baseado no modelo. Detectores determinísticos e intérprete produzem propostas tipadas (`NameUpdateIntent` ou `AddMemoryIntent`); `PendingMemoryEdit` e `PendingMemoryAdd` mantêm confirmação e cancelamento locais. O CLI apresenta e confirma a proposta sem acessar repository ou datasource, e `MemoryService` permanece responsável pela persistência real. Exclusão natural, edição genérica e captura automática continuam `planned`.
