...
A primeira ação externa controlada abre uma pasta já existente do workspace no Explorador de Arquivos do Windows. A interpretação natural produz apenas `OpenWorkspaceLocationIntent`; a capability resolve e confina o caminho localmente, registra um snapshot do diretório e só entrega esse alvo a um `LocationLauncher` injetado depois de confirmação explícita no CLI. Antes da execução, o alvo é validado novamente e uma troca invalida a proposta. O adaptador Windows usa o caminho absoluto de `explorer.exe`, argumentos separados e `shell=False`. A confirmação textual comum foi extraída para o CLI porque memória e desktop agora são consumidores reais; propostas e efeitos continuam em handlers específicos, sem registry, planner ou framework genérico de ações.
