...
A edição natural de memória implementada neste estágio é determinística e restrita à mudança do nome de Gustavo. `packages/conversation` reconhece somente dois padrões textuais explícitos, localiza candidatas por conteúdo e representa a proposta como `PendingMemoryEdit`; `MemoryService` continua responsável pela mutação validada por ID e snapshot. O CLI apresenta e confirma a proposta, sem acessar repository ou datasource. Esse fluxo não usa o modelo e pode ser reutilizado por uma interface de voz futura que forneça texto. Interpretação probabilística por modelo permanece `planned`.
