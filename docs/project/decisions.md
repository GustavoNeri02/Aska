...
`run_project_lint` representa exclusivamente `python -m ruff check .` na raiz do workspace. A proposal não possui parâmetros; paths, opções, `--fix`, formatação e seleção de comandos permanecem indisponíveis. O runner usa argv separado, `shell=False`, interpretador resolvido, cwd fixo, timeout e captura limitada. Snapshot e confirmação são locais, enquanto proposta, cancelamento e resultado viram eventos tipados apresentados pela IA. A infraestrutura de processo é compartilhada apenas no limite concreto do módulo Python; status e política permanecem específicos de lint. O pedido explícito e sem parâmetros possui fast path determinístico; paráfrases continuam no roteador semântico. Resultados executáveis exigem que a resposta estruturada da IA reconheça exatamente o status local; divergências são rejeitadas e não entram no histórico.
