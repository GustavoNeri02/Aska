...
### Filesystem read-only confinado ao workspace — `implemented`

O filesystem read-only opera dentro de um workspace explicitamente configurado e resolvido no composition root por `ASKA_WORKSPACE_ROOT`, usando o diretório atual como padrão. A leitura aceita um único arquivo textual UTF-8 de até 64 KiB. Caminhos explícitos são extraídos deterministicamente; um gate local evita interpretação adicional em mensagens comuns e, quando acionado, `ModelFileIntentInterpreter` aceita somente JSON estrito para leitura ou listagem. O modelo não recebe conteúdo prévio, não acessa o filesystem e não concede permissões.

`ReadTextFileCapability`, em `capabilities/filesystem`, retorna resultados tipados e resolve o caminho localmente, rejeitando `file://`, caminhos absolutos POSIX ou Windows, travessia, symlinks que escapem do workspace, diretórios, arquivos ausentes, vazios, binários, não UTF-8 ou acima do limite. Pedidos com caminho explícito usam extração determinística antes do interpretador; variações naturais continuam limitadas por gate e JSON estrito.
...
Além de comandos diretos, perguntas de conteúdo sobre os documentos conhecidos `README`, `AGENTS`, `roadmap` e decisões são detectadas local e deterministicamente. A referência ao documento e uma forma interrogativa, predicado de conteúdo ou ação direta devem estar presentes; menções genéricas não acionam leitura. Pedidos de localização com nome de arquivo explícito são determinísticos; demais pedidos com expressão clara de localização e referência a arquivo ou documento podem gerar somente uma intenção estruturada de listagem. Uma listagem vazia não cria contexto de arquivo nem permite seleção pelo modelo; seu evento tipado é enviado ao provider apenas para apresentação natural. Os fluxos reutilizam descoberta por nome, confinamento e contexto temporário existentes, sem conceder ao modelo decisão de acesso ou leitura durante a localização.

### Recuperação lexical seletiva de memórias — `implemented`

`TextMemoryRetriever` pertence a `packages/conversation`, pois relevância é uma política do consumidor de contexto e não do repositório responsável por persistência e CRUD. Ele normaliza caixa e acentos, remove termos comuns, pontua sobreposição textual e retorna no máximo cinco memórias, preservando a ordem persistida em empates. A consulta combina a mensagem atual com até dois turnos recentes. Sem correspondência, nenhuma memória é enviada ao modelo, e a identidade exige que ele admita não saber em vez de inventar fatos pessoais. Não há embeddings, busca vetorial, dependência nova ou alteração no JSON.

`ConversationService` registra apenas a seleção usada na última resposta concluída. `MemorySelection` contém `MemoryMatch` com score, termos correspondentes e classificação local em `exact`, `plural` ou `related`; a última exige radical comum de pelo menos seis caracteres e pequena diferença de comprimento. Uma pergunta explícita sobre quais memórias foram usadas ou por que foram selecionadas produz `memory_usage_report` com exatamente esses conteúdos e motivos, sem IDs e sem recalcular relevância. A IA formula a resposta, mas deve reconhecer domínio e tipo no envelope e apresentar o evento diretamente. Esse estado dura somente a sessão. Sinônimos, pesos configuráveis, orçamento por tokens, explicações semânticas e recuperação vetorial continuam `planned`.
...
`SearchTextIntent` e seu intérprete pertencem a `packages/conversation/natural_search.py`; termos entre aspas usam extração determinística e paráfrases claras passam por gate próprio e JSON estrito. `NaturalFileSearchHandler` permanece separado do handler de leitura/listagem e roda antes dele no CLI. Zero resultados e falhas tornam-se eventos locais tipados apresentados pela IA; somente correspondências não vazias entram como contexto temporário não confiável para a resposta atual e não são copiadas para o histórico.

### Sugestão aproximada de nome sem seleção automática — `implemented`

Quando uma localização explícita por nome e extensão não encontra resultado exato, o CLI pode sugerir caminhos semelhantes da mesma extensão. A comparação é local e pura, normaliza caixa e acentos, reconhece plurais simples e usa `difflib.SequenceMatcher` com corte e quantidade limitados. Sugestões não são leitura, busca semântica ou autorização: o modelo não participa da seleção, nenhum arquivo é escolhido ou aberto automaticamente e todas as candidatas já passaram pela listagem confinada ao workspace. O provider recebe apenas o evento tipado para produzir a apresentação natural.
...
A primeira execução de processo não oferece terminal genérico. `run_project_tests` é uma proposal sem subconjunto, comando, argumentos, caminho ou timeout. Ela representa exclusivamente a suíte inteira; um pedido por primeiro teste, arquivo, nome ou opção deve resultar em reply explicando a limitação, não em ampliação silenciosa de escopo. Quando essa reply oferece a suíte inteira, inclui `offer` tipada mantida pelo `ConversationService`; aceitações posteriores são interpretadas contra o estado estruturado, não contra uma lista de frases.

A implementação local fixa `python -m pytest -q`, usa o interpretador resolvido do ambiente do Aska, restringe o cwd à raiz configurada do workspace, revalida seu snapshot depois da confirmação e executa argv separado com `shell=False`. Timeout, falha de inicialização, exit code, stdout, stderr e truncamento são resultados locais autoritativos, nunca fatos inventados pelo modelo. Proposta, resultado e cancelamento produzem eventos estruturados usados pela IA para gerar a única apresentação visível. O histórico associa essa fala ao evento para sustentar follow-ups. Comando, diretório, timeout e estado pendente permanecem definidos localmente mesmo quando apresentados em linguagem natural pela Aska.

### Ruff check como operação executável fixa — `implemented`

`run_project_lint` representa exclusivamente `python -m ruff check .` na raiz do workspace. A proposal não possui parâmetros; paths, opções, `--fix`, formatação e seleção de comandos permanecem indisponíveis. O runner usa argv separado, `shell=False`, interpretador resolvido, cwd fixo, timeout e captura limitada. Snapshot e confirmação são locais, enquanto proposta, cancelamento e resultado viram eventos tipados apresentados pela IA. Testes e lint compartilham somente target fixo de workspace, validação, snapshot, revalidação e truncamento; comandos, status e política permanecem específicos. Desktop fica fora dessa extração por possuir alvo e efeito diferentes. O pedido explícito e sem parâmetros possui fast path determinístico; paráfrases continuam no roteador semântico. Resultados executáveis exigem que a resposta estruturada da IA reconheça exatamente o status local; divergências são rejeitadas e não entram no histórico.

### Código produz fatos; somente o modelo fala como Aska — `implemented`

No CLI de produção, handlers devolvem `HandlerResult` e não recebem `output_writer` nem `ConversationService`. O app transforma esses fatos em `ConversationEvent` ou contexto documental; a apresentação natural gerada pelo provider é a única saída conversacional e recebe o prefixo `Aska`. Somente a borda do CLI conhece o writer. Isso cobre memória natural e literal, filesystem, busca, desktop, terminal e capability indisponível.

Banner e lifecycle não simulam uma segunda voz. Propostas, recusas, confirmações e resultados continuam deterministicamente estruturados, porém sua apresentação pertence à Aska. Falhas do próprio provider, workspace inválido ou envelopes irrecuperáveis usam o fallback explícito `Erro`, pois não existe resposta válida do modelo e atribuí-las à Aska seria falso. O payload factual dos handlers permanece interno e poderá evoluir para modelos mais específicos quando cada domínio justificar.

Decisões conversacionais e apresentações de eventos com envelope inválido recebem uma única nova tentativa com instrução corretiva e o mesmo contexto. Eventos sensíveis — confirmação requerida, relatório de memórias usadas, comando inválido e resultado executável — exigem reconhecimento exato de domínio e tipo; resultados executáveis também reconhecem o status. Na apresentação de uma proposal já criada, a correção proíbe gerar outra `capability_proposal`: ela pode apenas formular a confirmação. A primeira resposta inválida não entra no histórico; se a segunda também violar o contrato, o erro técnico permanece explícito. Essa recuperação não executa proposals nem converte texto livre em autorização.
