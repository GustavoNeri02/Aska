...
A implementação local fixa `python -m pytest -q`, usa o interpretador resolvido do ambiente do Aska, restringe o cwd à raiz configurada do workspace, revalida seu snapshot depois da confirmação e executa argv separado com `shell=False`. Timeout, falha de inicialização, exit code, stdout, stderr e truncamento são resultados do sistema, nunca afirmações do modelo. Resultado e cancelamento produzem `ConversationEvent`: fatos brutos são exibidos localmente e enviados como contexto autoritativo para a IA gerar a fala natural. O histórico associa essa fala ao evento para sustentar follow-ups sem transformar texto pronto do handler em voz da Aska. O cartão de confirmação permanece local e determinístico.

### Código produz fatos; somente o modelo fala como Aska — `implemented`

No CLI de produção, saídas de handlers que não vieram do modelo são sempre rotuladas como `Sistema`. `TurnOutput` agrupa esses fatos por turno e cria um único `ConversationEvent`; a apresentação natural é gerada pelo provider e somente ela recebe o prefixo `Aska`. Isso cobre memória natural e literal, filesystem, busca, desktop, terminal e capability indisponível. Respostas já geradas não causam uma segunda apresentação.

Banner, lifecycle, uso de comandos, cartões de proposta e confirmação, recusas e resultados brutos continuam determinísticos como interface do `Sistema`. Falhas do próprio provider ou envelopes inválidos também são fallbacks de `Sistema`, pois não existe modelo disponível e atribuí-los à Aska seria falso. O payload factual de handlers legados ainda pode conter texto descritivo interno, mas ele não é apresentado como voz da Aska e será substituído por fatos mais estruturados quando cada domínio tiver necessidade concreta.
