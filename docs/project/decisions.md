...
`ConversationService` registra apenas a seleção usada na última resposta concluída. Uma pergunta explícita sobre quais memórias foram usadas produz `memory_usage_report` com exatamente esses conteúdos, sem IDs ou metadados e sem recalcular relevância. A IA formula a resposta, mas deve reconhecer domínio e tipo no envelope e listar o evento diretamente. Esse estado dura somente a sessão. Sinônimos, pesos, orçamento por tokens, explicações causais e recuperação semântica continuam `planned`.
