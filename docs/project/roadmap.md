...
- O roteamento semântico de proposals está `implemented` com catálogo fechado e JSON estrito. Frases exatas são somente fast paths; paráfrases podem gerar `OpenWorkspaceLocationProposal`, enquanto políticas e execução permanecem locais. A redução das duas chamadas de modelo em conversa comum continua `planned`.
...
O primeiro recorte executável também está `implemented`: abrir uma pasta existente do workspace no Explorador do Windows. Um roteador semântico provider-agnostic conhece um catálogo fechado, atualmente com apenas essa proposal, e recebe as mensagens que não foram resolvidas por fluxos anteriores. O modelo pode interpretar paráfrases e propor a action ou `none`; confinamento, proposta, confirmação, snapshot e revalidação são locais. Detectores de frases exatas existem somente como fast paths. Esse recorte não permite escolher executáveis, passar argumentos livres, abrir arquivos, usar shell, controlar mouse ou teclado nem executar comandos. A interação desktop geral da Fase 6 continua `planned`; o checkup manual desta operação está pendente.
