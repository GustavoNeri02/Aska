...
- Recuperação seletiva de memória está `implemented` com ranking lexical local explicável, limite de cinco resultados, mensagem atual e até dois turnos recentes. Correspondências exatas, plurais simples e radicais longos conservadores possuem pesos distintos. Somente conteúdos selecionados entram no contexto; score, termos e tipos de correspondência da última resposta podem ser consultados naturalmente pelo usuário. Sinônimos, relevância semântica e orçamento por tokens continuam `planned`.
...
- Seleção lexical explicável está `implemented`; orçamento por tokens, sinônimos, relevância semântica e compactação permanecem `planned`.
