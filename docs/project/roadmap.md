...
O primeiro recorte executável também está `implemented`: abrir uma pasta existente do workspace no Explorador do Windows. O modelo pode somente interpretar uma intenção limitada; confinamento, proposta, confirmação, snapshot e revalidação são locais. Esse recorte não permite escolher executáveis, passar argumentos livres, abrir arquivos, usar shell, controlar mouse ou teclado nem executar comandos. A interação desktop geral da Fase 6 continua `planned`; o checkup manual desta operação está pendente.
