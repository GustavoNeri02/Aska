...
O CLI conversa com o modelo local por meio do Ollama, mantém o histórico da sessão, oferece memória persistente explícita em JSON e pode ler arquivos textuais conhecidos, descobrir caminhos ou buscar texto literal com segurança dentro do workspace. Conteúdo, listagens e resultados de busca são usados somente como contexto temporário. Persistent Memory está `implemented`; Tools and capabilities está `in_progress` com esse recorte read-only.
